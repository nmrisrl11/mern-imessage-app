...
//! Define a strict tuple for [clerkId, fullName, email, profilePic]
type SeedUserTuple = [string, string, string, string];

const seedUsers: SeedUserTuple[] = [
...
async function seedDatabase(): Promise<void> {
	...
	//! Mongoose infers the operation type from the Model
	...
	console.log(`Seeded users. Inserted: ${result.upsertedCount}, updated: ${result.modifiedCount}, matched: ${result.matchedCount}`);
}

seedDatabase()
	//! Strongly type the catch block without using 'any'
	.catch((error: unknown) => {
